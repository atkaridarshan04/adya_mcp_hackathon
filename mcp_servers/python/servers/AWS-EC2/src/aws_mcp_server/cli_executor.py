...
async def check_aws_cli_installed() -> bool:
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(*cmd_parts, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, env=env)
...
async def execute_aws_command(command: str, timeout: int | None = None, credentials: dict | None = None) -> CommandResult:
    ...
    if is_pipe_command(command):
        return await execute_pipe_command(command, timeout, credentials)
    ...
    # Set up environment with credentials if provided
    env = None
    if credentials:
        import os
        env = os.environ.copy()
        ...
        if "aws_region" in credentials:
            ...
            # Also override the region in command if provided
            if "--region" not in command:
                command = f"{command} --region {credentials['aws_region']}"

    try:
        # Clean up escaped quotes in the command
        if '\"' in command:
            command = command.replace('\"', '"')
            logger.debug(f"Cleaned command: {command}")
...
async def execute_pipe_command(pipe_command: str, timeout: int | None = None, credentials: dict | None = None) -> CommandResult:
...
async def get_command_help(service: str, command: str | None = None) -> CommandHelpResult:
    ...
    try:
        ...
        result = await execute_aws_command(cmd_str, None, None)
