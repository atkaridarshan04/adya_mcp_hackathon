...
#         sys.exit(1)
    logger.info("Startup checks completed (AWS CLI check bypassed for dynamic credentials)")
...
async def aws_cli_pipeline(
    ...
    credentials: dict | None = Field(description="AWS credentials", default=None),
    ctx: Context | None = None,
) -> CommandResult:
    ...
    try:
        # Get credentials from either parameter
        creds = credentials
        result = await execute_aws_command(command, timeout, creds)
